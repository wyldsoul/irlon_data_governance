...
Implemented as reviewed SQL only: `sql/sbe37_public_parameter_suppression.sql`. It has not been run on production.

`rejected_observations.qc_flag` is deliberately lowercase, unquoted PostgreSQL naming, matching every existing `qc_*` column. It is an `integer`, the live type used by SBE37/public QC columns and by `update_rollup`/`qartod_update_log`. `3` is the established accepted/good convention; the registry accepts only `1` or `2`, records the code chosen by the reviewer, and writes that same code to the paired public `qc_<parameter>` column while NULLing the public value. The current database does not document a more granular semantic distinction between `1` and `2`; selecting which one applies remains an operator/QC-policy decision.

Identity is `(source_table, source_station, serial_number_sbe37, m_date, public_parameter)`. A range was intentionally not retained: it would make an already ambiguous public row identity broader without solving the absence of a public serial number. Bulk range rejection can be a reviewed follow-up that expands exact decisions explicitly.
...
1. Obtain explicit production approval, choose and document the meaning of QC values `1` versus `2`, and schedule a maintenance window. Do not create the registry from archive history.
