...
BEGIN
 ...
 IF NOT EXISTS (SELECT 1 FROM seabird_sunav2 WHERE station=p_source_station AND m_date=p_m_date) THEN RAISE EXCEPTION 'SUNA v2 source observation not found'; END IF;
 v_serial:=sunav2_resolve_serial(p_source_station,p_m_date);
 INSERT INTO rejected_observations(source_table,source_row_id,public_table,source_station,public_station,instrument_type,instrument_serial,m_date,public_parameter,qc_flag,rejection_reason,rejected_by,source_query_or_script)
 VALUES ('seabird_sunav2',(SELECT row_id::bigint FROM seabird_sunav2 WHERE station=p_source_station AND m_date=p_m_date),'seabird_sunav2',p_source_station,v_public,'SUNA v2',v_serial,p_m_date,p_public_parameter,p_qc_flag,p_rejection_reason,p_rejected_by,p_source_query_or_script) RETURNING rejection_id INTO v_id;
...
BEGIN
 ...
 WHERE active AND source_table='seabird_sunav2' AND public_table='seabird_sunav2' AND instrument_type='SUNA v2' AND source_station=p_source_station AND m_date=p_m_date AND public_parameter=p_public_parameter;
...
DECLARE v_source varchar;r record;
BEGIN
 IF NEW.station !~ '^.+-WQ$' THEN RETURN NEW; END IF; v_source:=sunav2_source_station(NEW.station);
 IF EXISTS(SELECT 1 FROM rejected_observations WHERE active AND source_table='seabird_sunav2' AND public_table='seabird_sunav2' AND instrument_type='SUNA v2' AND source_station=v_source AND public_station=NEW.station AND m_date=NEW.m_date)
    AND NOT EXISTS(SELECT 1 FROM seabird_sunav2 WHERE station=v_source AND m_date=NEW.m_date) THEN RAISE EXCEPTION 'SUNA v2 public write has no matching private observation'; END IF;
 FOR r IN SELECT * FROM rejected_observations WHERE active AND source_table='seabird_sunav2' AND public_table='seabird_sunav2' AND instrument_type='SUNA v2' AND source_station=v_source AND m_date=NEW.m_date LOOP
...
BEGIN
 IF NEW.station !~ '^_.+_SUNA$' THEN RETURN NEW; END IF;
 IF EXISTS(SELECT 1 FROM rejected_observations WHERE active AND source_table='seabird_sunav2' AND public_table='seabird_sunav2' AND instrument_type='SUNA v2' AND source_station=NEW.station AND m_date=NEW.m_date) THEN PERFORM sunav2_apply_active_rejections(NEW.station,NULL,NEW.m_date); END IF;
 RETURN NEW;
END $$;
